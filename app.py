...
<div class="card">
    <h3>Performance</h3>
    <p>{"★"*int(avg_rating)}{"☆"*(5-int(avg_rating))}</p>
</div>
...
<div class="card">
    ...
    {''.join([f"<p>⭐ {f['rating']} - '{f['comment']}'</p>" for f in feedbacks])}
</div>
...
<script>
function startClass(){{
    window.open("{teacher["link"]}", "_blank");
}}

// GRAPH
const data = {ratings if ratings else [0]};
